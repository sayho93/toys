const FileRepository = ({DataSourceMariaDB}) => {
    const addFile = async ({userId = 0, originName, path, shortPath, size, width, height, ext, desc = ''}) => {
        ...
        const [res] = await DataSourceMariaDB.exec(query, [userId, originName, path, shortPath, size, width, height, ext, desc])
        return res.insertId
    }

    const getFile = async ({id}) => {
        const query = `SELECT * FROM file WHERE id = ?`
        const [res] = await DataSourceMariaDB.exec(query, [id])
        return res
    }
